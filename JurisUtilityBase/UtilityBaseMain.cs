...
//            listBoxCompanies.SelectedIndexChanged += listBoxCompanies_SelectedIndexChanged;
        ...
        private void DoDaFix()
        {
            ...
            List<string> cashExps = getCashExps().ToList();
            ...
            if (cashExps != null) // did we get any cash exps?
            {
                ...
                if (!incomeAcct .Equals("None"))
                {
                    ...
                    if (!incomeAcct.Equals("None")) //does this adj account exist?
                    {
                        ...
                        processAccts(cashExps, adjAccount, "A"); //adjustment
                        UpdateStatus("Expense Accounts updated.", 1, 1);
        ...
        private List<string> getCashExps()
        {
            string sql = "SELECT ExpCdCode from ExpenseCode where ExpCdExpType = 'C'";
        ...
        private string getChtSysNbr(string accountCode)
        {
            string sql = "select ChtSysNbr from ChartOfAccounts where dbo.jfn_FormatChartOfAccount(ChartOfAccounts.ChtSysNbr) = '" + accountCode + "'";
            DataSet dd = _jurisUtility.RecordsetFromSQL(sql);

            if (dd != null && dd.Tables[0].Rows.Count > 0)
                return dd.Tables[0].Rows[0][0].ToString();
            else
                return "None";
        }

        private void processAccts(List<String> codes, string acct, string IAFlag)
        {
            foreach (string code in codes)
            {
                string sql = "SELECT ECGAAcct from ExpCodeGLAcct where ECGAExpCode = '" + code + "' and ECGAType = '" + IAFlag + "'";
                DataSet dd = _jurisUtility.RecordsetFromSQL(sql);

                if (dd != null && dd.Tables[0].Rows.Count > 0)
                {
                    sql = "update ExpCodeGLAcct set ECGAAcct = " + acct + " where ECGAExpCode = '" + code + "' and ECGAType = '" + IAFlag + "' ";
